...
def dump_mongo(corpora, feat_name, n_topics, acc, pred, all_pred, all_acc, all_nmi, all_ari):
    ...
    results = cluster_db.vn_clusters
...
if __name__ == '__main__':
    ...
    all_feat = {'elmo_max':feat_max, 'elmo_mean':feat_mean}
    #
    trial_num = 10
    #
    max_topic = 30
    ...
    for n_topics in range(n_clusters, max_topic + step, step):
        for feat_name, feat in all_feat.items():
            for func_name, feat_trans_func in feat_func_dict.items():
                ...
                for i in range(trial_num):
                    pred = cluster_alg(feat_tmp, n_topics)
