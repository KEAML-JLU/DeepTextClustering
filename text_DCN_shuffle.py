...
class DCN(object):
        ...
        for epoch in range(self.max_epochs):
            ...
            if epoch == 1:
                if self.init_num < 0:
                    ...
                    count = {i: self.init_num for i in range(self.n_clusters)}

            for index in range(0, data_size, batch_size):
                feat_batch = Variable(torch.from_numpy(feat[shuffle_idx[index: index+batch_size]]))
                idx_batch = idx[shuffle_idx[index: index+batch_size]]
                ...
                if epoch >= 1:
                    ...
                    idx[shuffle_idx[index: index+batch_size]] = tmp_idx_batch
    ...
    def batch_km(data, centers, count):
        ...
        if True:
            ...
            N = data.shape[0]
            tmp_idx = np.zeros(N, dtype=np.int64)
            for i in range(N):
                distance = np.linalg.norm(data[i,np.newaxis] - centers, axis=-1)
...
if __name__ == '__main__':
    ...
    def get_args():
        ...
        parser.add_argument('--init_num', type=int, default=100, help='max epochs')
