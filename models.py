...
# the intent to establishing the adaptor is building an easy-using forward function.
class InfersentAdaptor(nn.Module):

    def __init__(self, infersent):
        super(InfersentAdaptor, self).__init__()
        assert isinstance(infersent, BLSTMEncoder)
        self.infersent = infersent

    def forward(self, sentences, layer_norm=False):
        # sentences must be list of str
        # self.infersent.eval function will disable infersent's dropout.
        self.infersent.eval()
        # infersent.prepare_samples' param bsize is not used in function body, so it's set to an any value.
        sentences, lengths, idx_sort = self.infersent.prepare_samples(
            sentences, 20, tokenize=False, verbose=False)
        batch = Variable(self.infersent.get_batch(sentences))
        if self.is_cuda():
            batch = batch.cuda()
        hidden = self.infersent.forward((batch, lengths))
        if layer_norm:
            hidden = self.__layer_norm_torch(hidden)
        return hidden

    def __layer_norm_torch(self, features, eps=1e-6):
        # This is simple implement of layer normalzation. Affine transformation might be added in the future.
        features = (features - features.mean(dim=1, keepdim=True)) / (features.std(dim=1, keepdim=True) + eps)
        return features
    ...
    def is_cuda(self):
        return self.infersent.is_cuda()

    def encode(self, sentences, bsize=64, verbose=False, layer_norm=False, tokenize=False):
        features = self.infersent.encode(sentences, bsize, tokenize=False, verbose=verbose)
        if layer_norm:
            features = self.__layer_norm_numpy(features)
        return features

    def visualize(self, sent):
        return self.infersent.visualize(sent, tokenize=False)

    @staticmethod
    def __layer_norm_numpy(data, eps=1e-6):
        result = (data - data.mean(axis=1, keepdims=True)) / (data.std(axis=1, keepdims=True) + eps)
        return result


class ClusterNet(nn.Module):
    def __init__(self, init_cluster_centers):
        # init_cluster_centers is torch.Tensor
        super(type(self), self).__init__()
        self.n_clusters = init_cluster_centers.size(0)
        self.vec_dim = init_cluster_centers.size(1)
        self.cluster_centers = nn.Parameter(init_cluster_centers)

    def forward(self, input):
        assert self.vec_dim == input.size(1)
        input = input.unsqueeze(1)
        q = 1. / (1. + torch.sum(torch.pow(input - self.cluster_centers, 2), dim=2))
