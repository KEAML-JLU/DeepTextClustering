...
def dump_mongo(corpora, feat_name, n_topics, acc, pred, all_pred, all_acc, all_nmi, all_ari):
    ...
    tmp = {
            ...
            'ari_mean':ari_mean}
    print(tmp)
    with open('lda_results.txt','a') as f:
        import json
...
if __name__ == '__main__':
    ...
    for corpora_id in range(3, 7):
        ...
        feat = get_count_feat(sents)

        for K in Ks:
            n2alg = {0:'lda_cluster', 1:'lda_kmeans'}
            for j, alg in enumerate([lda_cluster_alg, lda_kmeans_cluster_alg]):
                trial_num = 10

                feat_name = n2alg[j] + '_' + str(K)
                best_acc = 0.0
                best_pred = None
                feat_tmp = feat
                all_pred = []
                ...
                for i in range(trial_num):
                    ...
                    pred = alg(feat, n_clusters, n_topics=K)
